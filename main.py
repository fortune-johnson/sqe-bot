"""
╔══════════════════════════════════════════════════════════════════════════════════╗
║   SYNTHETIC QUANT ELITE v4.3 — PRODUCTION (MINIMAL MESSAGES)                   ║
║   All internal logic unchanged. Only alert messages simplified.                ║
║   Spike alerts: confidence bar, window, ticks, hazard, risk warning.          ║
║   Trade alerts: direction, strength, source, entry, target, stop, risk warning.║
╚══════════════════════════════════════════════════════════════════════════════════╝
"""
import os
import asyncio
...
log = logging.getLogger("SQEv43_Prod")
...
# CREDENTIALS & CONFIG
# ============================================================================
TELEGRAM_TOKEN = "8308494145:AAEx4Cs3a90rvuym2Z7sguqUSeDwienYaxM"
DERIV_APP_ID   = "1089"
...
# Spike confidence filter (only send if confidence >= 95%)
...
def _empirical_move_cap(prices: np.ndarray, horizon: int,
                        cap_percentile: float = EMPIRICAL_CAP_PERCENTILE,
                        cap_multiplier: float = EMPIRICAL_CAP_MULTIPLIER,
                        min_windows: int = EMPIRICAL_MIN_WINDOWS) -> Optional[float]:
...
async def _safe_send_photo(bot, chat_id: int, photo, caption: str, **kwargs):
    try:
        if len(caption) <= MAX_CAPTION:
            return await bot.send_photo(chat_id, photo=photo, caption=caption,
                                        parse_mode=ParseMode.MARKDOWN, **kwargs)
        short = caption[:MAX_CAPTION].rsplit("\n", 1)[0]
        await bot.send_photo(chat_id, photo=photo, caption=short,
                             parse_mode=ParseMode.MARKDOWN, **kwargs)
        ...
        try:
            ...
            await bot.send_photo(chat_id, photo=photo, caption=plain[:MAX_CAPTION])
        except Exception as e2:
            log.error(f"send_photo fallback: {e2}")

async def _safe_send_message(bot, chat_id: int, text: str, **kwargs):
    MAX_MSG = 4000
    chunks = [text[i:i+MAX_MSG] for i in range(0, len(text), MAX_MSG)]
    for idx, chunk in enumerate(chunks):
        kw = kwargs.copy() if idx == 0 else {}
        try:
            await bot.send_message(chat_id, chunk, parse_mode=ParseMode.MARKDOWN, **kw)
        except Exception as e:
            if any(x in str(e).lower() for x in ["parse", "entity", "can't"]):
...
class TimeOfDayProfile:
    ...
    def _ensure(self, sym: str):
        if sym not in self._buckets:
            self._buckets[sym] = [TodBucket() for _ in range(TOD_BUCKETS)]

    def update(self, sym: str, win: bool, edge: float, ev: float, vol_dev: float,
               tick_rate: float, bucket: Optional[int] = None):
        self._ensure(sym)
...
class AssetProfile:
    ...
    alert_strength: List[str] = field(default_factory=lambda: ["STRONG", "MODERATE"])
    mc_paths: int = 10_000
...
PROFILE_VOL = AssetProfile(
    name="VOLATILITY", model_type="GBM", edge_min=0.12, direction_gap=0.003,
    ci_width_min=0.000001, signal_edge_min=0.10, strong_edge=0.25, strong_prob=0.58,
    strong_rr=2.0, moderate_edge=0.12, moderate_prob=0.54, moderate_rr=1.4,
    trade_enabled=True, min_target_pct=0.001, min_stop_pct=0.001, min_rr=1.4,
    target_percentile=65.0, stop_percentile=35.0, direction_bias="NONE", spike_enabled=False,
    alert_edge_min=0.12, alert_cooldown=360.0, alert_min_ticks=400, alert_min_rr=1.4,
    alert_strength=["STRONG","MODERATE"], mc_paths=10_000, sigma_multiplier_base=2.2,
    drift_signal_enabled=True, drift_tstat_min=2.0, drift_window=200, drift_edge_weight=0.55,
    vol_regime_enabled=True, vol_k_min=2.0, vol_k_max=2.8, resolve_threshold_pct=0.02, tod_enabled=True)

PROFILE_BOOM = AssetProfile(
    name="BOOM", model_type="JumpDiffusion", edge_min=0.25, direction_gap=0.010,
    ci_width_min=0.00001, signal_edge_min=0.15, strong_edge=0.40, strong_prob=0.58,
    strong_rr=2.0, moderate_edge=0.20, moderate_prob=0.54, moderate_rr=1.4,
    trade_enabled=True, min_target_pct=0.0025, min_stop_pct=0.005, min_rr=1.4,
    target_percentile=65.0, stop_percentile=35.0, direction_bias="BUY", spike_enabled=True,
    spike_min_prob=0.45, spike_min_magnitude=0.03, spike_lam_dev_min=8.0, spike_hazard_high=0.65,
    spike_is_primary=True, alert_edge_min=0.20, alert_cooldown=300.0, alert_min_ticks=400,
    alert_min_rr=1.4, alert_strength=["STRONG","MODERATE"], mc_paths=10_000, sigma_multiplier_base=2.2,
    drift_signal_enabled=False, vol_regime_enabled=False, resolve_threshold_pct=0.10, tod_enabled=True)

PROFILE_CRASH = AssetProfile(
    name="CRASH", model_type="JumpDiffusion", edge_min=0.25, direction_gap=0.010,
    ci_width_min=0.00001, signal_edge_min=0.15, strong_edge=0.40, strong_prob=0.58,
    strong_rr=2.0, moderate_edge=0.20, moderate_prob=0.54, moderate_rr=1.4,
    trade_enabled=True, min_target_pct=0.0025, min_stop_pct=0.005, min_rr=1.4,
    target_percentile=35.0, stop_percentile=65.0, direction_bias="SELL", spike_enabled=True,
    spike_min_prob=0.45, spike_min_magnitude=0.03, spike_lam_dev_min=8.0, spike_hazard_high=0.65,
    spike_is_primary=True, alert_edge_min=0.20, alert_cooldown=300.0, alert_min_ticks=400,
    alert_min_rr=1.4, alert_strength=["STRONG","MODERATE"], mc_paths=10_000, sigma_multiplier_base=2.2,
    drift_signal_enabled=False, vol_regime_enabled=False, resolve_threshold_pct=0.10, tod_enabled=True)
...
PROFILE_JUMP = AssetProfile(
    name="JUMP", model_type="JumpDiffusion", edge_min=0.15, direction_gap=0.005,
    ci_width_min=0.000005, signal_edge_min=0.12, strong_edge=0.28, strong_prob=0.57,
    strong_rr=2.0, moderate_edge=0.15, moderate_prob=0.54, moderate_rr=1.4,
    trade_enabled=True, min_target_pct=0.001, min_stop_pct=0.005, min_rr=1.4,
    target_percentile=62.0, stop_percentile=38.0, direction_bias="NONE", spike_enabled=False,
    spike_is_primary=False, alert_edge_min=0.15, alert_cooldown=180.0, alert_min_ticks=200,
    alert_min_rr=1.4, alert_strength=["STRONG","MODERATE"], mc_paths=10_000, sigma_multiplier_base=2.0,
    drift_signal_enabled=True, drift_tstat_min=1.8, drift_window=150, drift_edge_weight=0.45,
    vol_regime_enabled=False, resolve_threshold_pct=0.05, tod_enabled=True)

PROFILE_STEP = AssetProfile(
    name="STEP", model_type="Step", edge_min=0.10, direction_gap=0.002,
    ci_width_min=0.0000001, signal_edge_min=0.08, strong_edge=0.22, strong_prob=0.56,
    strong_rr=1.5, moderate_edge=0.12, moderate_prob=0.54, moderate_rr=1.4,
    trade_enabled=True, min_target_pct=0.0001, min_stop_pct=0.00005, min_rr=1.4,
    target_percentile=65.0, stop_percentile=35.0, direction_bias="NONE", spike_enabled=False,
    alert_edge_min=0.10, alert_cooldown=360.0, alert_min_ticks=300, alert_min_rr=1.4,
    alert_strength=["STRONG","MODERATE"], mc_paths=8_000, sigma_multiplier_base=1.0,
    drift_signal_enabled=True, drift_tstat_min=2.2, drift_window=300, drift_edge_weight=0.55,
    vol_regime_enabled=False, resolve_threshold_pct=0.01, tod_enabled=True)
...
SYMBOL_TUNES: Dict[str, SymbolTune] = {
    "R_10": SymbolTune(min_target_pct_override=0.001, min_rr_override=1.4, sigma_mult_override=1.2,
                       drift_tstat_override=2.0, drift_window_override=300, vol_k_min_override=1.8, vol_k_max_override=2.5),
    "R_25": SymbolTune(min_target_pct_override=0.0015, min_rr_override=1.4, sigma_mult_override=1.3,
                       drift_tstat_override=2.0, vol_k_min_override=1.9, vol_k_max_override=2.6),
    "R_50": SymbolTune(min_target_pct_override=0.002, min_rr_override=1.4, sigma_mult_override=1.6,
                       vol_k_min_override=1.8, vol_k_max_override=2.5),
    "R_75": SymbolTune(min_target_pct_override=0.002, min_rr_override=1.4, sigma_mult_override=1.8,
                       vol_k_min_override=2.0, vol_k_max_override=2.6),
    "R_100": SymbolTune(min_target_pct_override=0.003, min_rr_override=1.4, sigma_mult_override=2.2,
                        vol_k_min_override=2.2, vol_k_max_override=2.8),
    "R_250": SymbolTune(min_target_pct_override=0.005, min_rr_override=1.4, sigma_mult_override=2.5,
                        vol_k_min_override=2.3, vol_k_max_override=2.8),
    "1HZ10V": SymbolTune(min_target_pct_override=0.001, min_rr_override=1.4, sigma_mult_override=1.2,
                         drift_tstat_override=2.0, vol_k_min_override=1.8, vol_k_max_override=2.5),
    "1HZ25V": SymbolTune(min_target_pct_override=0.0015, min_rr_override=1.4, sigma_mult_override=1.3,
                         vol_k_min_override=1.9, vol_k_max_override=2.6),
    "1HZ50V": SymbolTune(min_target_pct_override=0.002, min_rr_override=1.4, sigma_mult_override=1.6,
                         vol_k_min_override=1.8, vol_k_max_override=2.5),
    "1HZ75V": SymbolTune(min_target_pct_override=0.002, min_rr_override=1.4, sigma_mult_override=1.8,
                         vol_k_min_override=2.0, vol_k_max_override=2.6),
    "1HZ100V": SymbolTune(min_target_pct_override=0.003, min_rr_override=1.4, sigma_mult_override=2.2,
                          vol_k_min_override=2.2, vol_k_max_override=2.8),
    "BOOM300N": SymbolTune(alert_cooldown_override=180.0, spike_min_prob_override=0.50),
    "BOOM500": SymbolTune(alert_cooldown_override=240.0, spike_min_prob_override=0.45),
    "BOOM600N": SymbolTune(alert_cooldown_override=270.0, spike_min_prob_override=0.45),
    "BOOM900": SymbolTune(alert_cooldown_override=360.0, spike_min_prob_override=0.42),
    "BOOM1000": SymbolTune(alert_cooldown_override=400.0, spike_min_prob_override=0.40),
    "CRASH300N": SymbolTune(alert_cooldown_override=180.0, spike_min_prob_override=0.50),
    "CRASH500": SymbolTune(alert_cooldown_override=240.0, spike_min_prob_override=0.45),
    "CRASH600N": SymbolTune(alert_cooldown_override=270.0, spike_min_prob_override=0.45),
    "CRASH900": SymbolTune(alert_cooldown_override=360.0, spike_min_prob_override=0.42),
    "CRASH1000": SymbolTune(alert_cooldown_override=400.0, spike_min_prob_override=0.40),
    "JD10": SymbolTune(alert_cooldown_override=60.0, mc_paths_override=8_000, drift_tstat_override=1.8, drift_window_override=100),
    "JD25": SymbolTune(alert_cooldown_override=90.0, drift_tstat_override=1.8),
...
def get_profile(sym: str) -> AssetProfile:
    cat = _cat(sym)
    return {"vol": PROFILE_VOL, "boom": PROFILE_BOOM, "crash": PROFILE_CRASH,
            "jump": PROFILE_JUMP, "step": PROFILE_STEP}.get(cat, PROFILE_VOL)
...
@njit(parallel=True, cache=True, fastmath=True)
def _jd_kernel(S0, mu, sigma, lam, jmean, jstd, jsign, dt, n_paths, n_steps, Z, U, Zj):
    paths = np.empty((n_paths, n_steps+1), dtype=np.float64)
    drift = (mu - 0.5*sigma*sigma)*dt
    vol = sigma*math.sqrt(dt)
    lam_dt = lam*dt
...
class PersistenceEngine:
    ...
    def _load(self):
        for fn in (PERSIST_FILE, "sqe_v42_state.pkl", "sqe_v41_state.pkl", "sqe_v40_state.pkl"):
...
class PatternMemory:
    ...
    def _load(self):
        for fn in (PATTERN_FILE, "pattern_memory_v42.pkl", "pattern_memory_v41.pkl", "pattern_memory_v40.pkl"):
            if os.path.exists(fn):
                try:
                    with open(fn, "rb") as f:
    ...
    def resolve(self, sym: str, price: float, bot=None, chats: Optional[Set[int]] = None):
        now = time.time()
        ...
        for r in self.records:
            ...
            if bot and chats:
                ...
                for cid in list(chats):
                    asyncio.ensure_future(_safe_send_message(bot, cid, msg))
        if changed:
            self._save()
...
# ============================================================================
# HYBRID TARGET REFINER (unchanged)
# ============================================================================
class HybridTargetRefiner:
    def refine(self, sym, S0, is_bull, mc_target, mc_stop, finals, prices, horizon, records):
        n_records = len([r for r in records if r.symbol == sym and r.resolved])
        ...
        if cold:
            if emp_cap is not None:
                ...
                return S0 * (1 + direction * emp_cap), S0 * (1 - direction * emp_cap * 0.40), "empirical_cold"
        ...
        try:
            ...
            if emp_cap is not None and abs(blended - S0) / S0 > emp_cap:
                direction = 1.0 if is_bull else -1.0
...
# ============================================================================
# DATA MANAGER (full, unchanged)
# ============================================================================
class DataManager:
    def __init__(self):
        self.buffers: Dict[str, deque] = {s: deque(maxlen=BUFFER_SIZE) for s in ALL_SYMBOLS.values()}
        self.timestamps: Dict[str, deque] = {s: deque(maxlen=BUFFER_SIZE) for s in ALL_SYMBOLS.values()}
        self.latest: Dict[str, float] = {}
        self._tick_times: Dict[str, deque] = {s: deque(maxlen=300) for s in ALL_SYMBOLS.values()}
        self._ws = None
        self._running = False
    ...
    def observed_tick_rate(self, sym: str) -> float:
        ...
        if len(times) < 10:
            cat = _cat(sym)
            return {"boom": 0.5, "crash": 0.5, "jump": 1.0, "step": 1.0}.get(cat, 0.5)
        diffs = np.diff(times)
        diffs = diffs[diffs > 0]
        if len(diffs) == 0:
            return 0.5
    ...
    async def _initialise(self):
        ...
        for sym in ALL_SYMBOLS.values():
            rid = await self._next_id()
            try:
                await self._ws.send(json.dumps({"ticks": sym, "subscribe": 1, "req_id": rid}))
...
# ============================================================================
# VOLATILITY REGIME ENGINE (unchanged)
# ============================================================================
class VolatilityRegimeEngine:
    def __init__(self, vm: VolatilityModel):
        self.vm = vm

    def _target_sigma_per_tick(self, sym: str, tpy: int = 86400 * 365) -> float:
        adv = ADVERTISED_VOL.get(sym, 0.0)
        if adv == 0:
            return 0.01 / math.sqrt(tpy)
        return adv / math.sqrt(tpy)

    def _normalized_momentum(self, r: np.ndarray, realized_sigma: float, window: int) -> float:
    ...
    def analyse(self, sym: str, lr: np.ndarray, S0: float, horizon: int,
                profile: AssetProfile, tod_mult: float = 1.0,
                window_override: Optional[int] = None, prices: Optional[np.ndarray] = None) -> VolRegimeResult:
        r = lr[np.isfinite(lr)]
        if len(r) < MIN_TICKS:
        ...
        vol_deviation = (realized_sigma - target_sigma) / max(target_sigma, 1e-10)
        ...
        k_adaptive = float(np.clip(
            profile.vol_k_min + dev_factor * (profile.vol_k_max - profile.vol_k_min),
        ...
        norm_mom_blended = float(np.clip(norm_mom_short * 0.65 + norm_mom_long * 0.35, -5.0, 5.0))
        norm_mom = norm_mom_short

        sigma_h = realized_sigma * math.sqrt(max(horizon, 1))
        stop_factor = {"COMPRESSED": 0.55, "NORMAL": 0.70, "EXPANDED": 0.85}.get(regime, 0.70)
        ...
        if prices is not None and len(prices) >= horizon * 2:
            ...
            if emp_cap is not None:
                ...
                if raw_up_move > emp_cap:
                    rr_ratio_up = raw_up_move / max((S0 - stop_down) / S0, 1e-10)
                    target_up = S0 * (1.0 + emp_cap)
                    clamped_stop = emp_cap / max(rr_ratio_up, 1e-10)
                    stop_down = S0 * (1.0 - min(clamped_stop, emp_cap * 0.8))
                if raw_down_move > emp_cap:
                    rr_ratio_down = raw_down_move / max((stop_up - S0) / S0, 1e-10)
                    target_down = S0 * (1.0 - emp_cap)
                    clamped_stop = emp_cap / max(rr_ratio_down, 1e-10)
                    stop_up = S0 * (1.0 + min(clamped_stop, emp_cap * 0.8))
        ...
        long_agrees = (norm_mom_long > 0 and norm_mom_short > 0) or (norm_mom_long < 0 and norm_mom_short < 0)
        p_significant = pvalue < p_thresh

        mom_score = float(np.clip(abs(norm_mom) / max(t_thresh_eff * 1.5, 1), 0.0, 1.0))
        ewma_score = float(np.clip(abs(ewma_mom), 0.0, 1.0))
        regime_bonus = {"EXPANDED": 1.35, "NORMAL": 1.00, "COMPRESSED": 0.80}.get(regime, 1.0)
        long_bonus = 1.15 if long_agrees else 0.90
        raw_edge = (mom_score * 0.65 + ewma_score * 0.35) * regime_bonus * long_bonus * tod_mult
        edge_score = float(np.clip(raw_edge, 0.0, 1.0))

        edge_ok = edge_score >= profile.signal_edge_min
        strong_enough = edge_score >= 0.18
        if edge_ok and (p_significant or strong_enough) and (is_bullish or is_bearish):
            direction = "bullish" if is_bullish else "bearish"
        else:
            direction = "neutral"

        if edge_score >= profile.strong_edge and abs(norm_mom) >= t_thresh_eff * 2.0 and pvalue < 0.10:
            strength = "STRONG"
        elif edge_score >= profile.moderate_edge and abs(norm_mom) >= t_thresh_eff and pvalue < 0.25:
            strength = "MODERATE"
        else:
            strength = "WEAK"
    ...
    def _neutral(self, sym: str, S0: float, horizon: int,
                 profile: AssetProfile, tod_mult: float) -> VolRegimeResult:
...
class DriftEngine:
    ...
    def _rolling_tstat(self, r: np.ndarray, window: int) -> Tuple[float, float]:
        recent = r[-window:] if len(r) >= window else r
        if len(recent) < 20:
            return 0.0, 1.0
    ...
    def analyse(self, sym: str, lr: np.ndarray, S0: float, profile: AssetProfile,
                tod_mult: float = 1.0, window_override: Optional[int] = None) -> DriftSignal:
        r = lr[np.isfinite(lr)]
        if window_override is not None:
            window = max(min(window_override, len(r) // 2), 30)
        else:
        ...
        if len(r) < 30:
            return DriftSignal(
                ...
                window_used=window, edge_score=0.0, signal_strength="WEAK"
            )
        ...
        tstat_score = float(np.clip(abs(tstat) / max(profile.drift_tstat_min * 2, 1), 0.0, 1.0))
        momentum_score = float(np.clip(abs(momentum), 0.0, 1.0))
        regime_bonus = {"TRENDING": 1.40, "VOLATILE": 1.15, "NORMAL": 1.00, "RANGING": 0.65}.get(regime, 1.0)
        agree_bonus = 1.20 if agrees else 0.80
        raw_edge = (tstat_score * 0.55 + momentum_score * 0.45) * regime_bonus * agree_bonus * tod_mult
        edge_score = float(np.clip(raw_edge, 0.0, 1.0))
        ...
        if t_significant and p_significant and edge_score >= profile.signal_edge_min and (is_bullish or is_bearish):
            direction = "bullish" if is_bullish else "bearish"
        else:
            direction = "neutral"

        if edge_score >= profile.strong_edge and abs(tstat) >= t_threshold * 1.5 and agrees and pvalue < 0.05:
            strength = "STRONG"
        elif edge_score >= profile.moderate_edge and abs(tstat) >= t_threshold and pvalue < 0.15:
            strength = "MODERATE"
        else:
            strength = "WEAK"
...
# ============================================================================
# JUMP-DIFFUSION MODEL (unchanged)
# ============================================================================
class JumpDiffusionModel:
    ...
    def _hazard_intensity_bayesian(self, lr: np.ndarray, lam_posterior: float,
                                   expected_lam: float, tod_mult: float = 1.0) -> float:
        if len(lr) < 20 or lam_posterior <= 0:
            return 0.0
        is_j, _ = self.detect(lr)
        jump_idx = np.where(is_j)[0]
        ticks_since = len(lr) if len(jump_idx) == 0 else len(lr) - int(jump_idx[-1])
    ...
    def _bayesian_lambda(self, n_jumps: int, n_obs: int, expected_lam: float,
    ...
    def _spike_magnitude_percentiles(self, jumps: np.ndarray) -> Tuple[float, float]:
        abj = np.abs(jumps)
        if len(abj) < 3:
            return 0.02, 0.03
    ...
    def _counter_drift(self, lr: np.ndarray, is_jump: np.ndarray) -> float:
        diff_r = lr[~is_jump]
        if len(diff_r) < 20:
            return 0.0
        try:
    ...
    def _fit_core(self, sym, r, is_j, jumps, sign) -> JumpParams:
        ...
        lam_dev = ((lam_mle - t_lam) / (t_lam + 1e-12) * 100) if t_lam > 0 else 0.0
    ...
    def ticks_to_next_range(self, p: JumpParams, tick_rate: float) -> Tuple[int, int, str, str, str]:
        lam = max(p.lam_posterior, p.lam, 1e-10)
        if lam <= 0:
...
# ============================================================================
# SPIKE ENGINE (simplified format)
# ============================================================================
class SpikeEngine:
    def __init__(self, jm: JumpDiffusionModel):
        self.jm = jm

    def assess(self, sym: str, jp: JumpParams, horizon: int, tick_rate: float,
               tod_mult: float = 1.0) -> Optional[SpikeAlert]:
        profile = effective_profile(sym)
        ...
        c3 = (abs(jp.lam_deviation) >= profile.spike_lam_dev_min or
              jp.recent_cluster > jp.expected_lam * 1.2 or
              jp.inter_arrival_cv > 1.15 or
              jp.hazard_intensity >= profile.spike_hazard_high)
    ...
    def format_standalone(self, sa: SpikeAlert) -> str:
        ...
        lines = [
            ...
            f"Hazard={sa.hazard_intensity:.0%} HIGH (Bayesian nonlinear)\n",
            f"{RISK_MSG}"
        ]
        return "\n".join(lines)

# ============================================================================
# TRADE SETUP BUILDER (simplified format_setup)
# ============================================================================
class TradeSetupBuilder:
    @staticmethod
    def _signal_strength(edge: float, prob: float, rr: float, profile: AssetProfile) -> str:
        if edge >= profile.strong_edge and prob >= profile.strong_prob and rr >= profile.strong_rr:
            return "STRONG"
        if edge >= profile.moderate_edge and prob >= profile.moderate_prob and rr >= profile.moderate_rr:
            return "MODERATE"
        return "WEAK"

    @staticmethod
    def build(mc: MCResult, timeframe_label: str, horizon_ticks: int, regime: str = "NORMAL",
              profile: Optional[AssetProfile] = None, drift: Optional[DriftSignal] = None,
              vol_regime: Optional[VolRegimeResult] = None, tod_mult: float = 1.0) -> Optional[TradeSetup]:
        if profile is None:
        ...
        if profile.direction_bias == "BUY":
            ...
            is_bull = False
            signal_source = "PROFILE"
        elif vol_regime is not None and profile.vol_regime_enabled and vol_regime.signal != "neutral":
            is_bull = (vol_regime.signal == "bullish")
            signal_source = "VOL_REGIME"
        elif vol_regime is not None and profile.vol_regime_enabled and vol_regime.signal == "neutral":
            return None
        elif drift is not None and profile.drift_signal_enabled and drift.direction != "neutral":
        ...
        # Target and stop
        if vol_regime is not None and profile.vol_regime_enabled and vol_regime.signal != "neutral":
            if is_bull:
        ...
        try:
            ...
            if mc_stop_pct > 1e-6 and mc_target_pct >= MIN_TARGET_MOVE * 100:
        ...
        if vol_regime is not None and profile.vol_regime_enabled and vol_regime.signal != "neutral":
            ...
            w_m = 1.0 - w_v
            combined_edge = float(np.clip(vol_regime.edge_score * w_v + mc.edge_score * w_m, 0.0, 1.0))
            prob_boost = vol_regime.edge_score * 0.12
            prob_t = float(np.clip(max(prob_mc + prob_boost, 0.54), 0.50, 0.72))
            if signal_source == "MC":
                signal_source = "VOL_REGIME"
        elif drift is not None and profile.drift_signal_enabled and drift.direction != "neutral":
            w_d = profile.drift_edge_weight
            w_m = 1.0 - w_d
            combined_edge = float(np.clip(drift.edge_score * w_d + mc.edge_score * w_m, 0.0, 1.0))
            prob_boost = drift.edge_score * 0.08
            prob_t = float(np.clip(max(prob_mc + prob_boost, 0.53), 0.50, 0.70))
        ...
        strength = TradeSetupBuilder._signal_strength(combined_edge, prob_t, rr_ratio, profile)

        return TradeSetup(
            direction=direction, entry=S0, target=target, invalidation=inv,
            target_pct=target_pct, stop_pct=stop_pct, rr_ratio=rr_ratio,
            prob_target=prob_t, prob_stop=prob_s, edge_pct=edge_pct,
            expected_value=ev, signal_strength=strength,
            timeframe_label=timeframe_label, horizon_ticks=horizon_ticks,
            regime=regime, profile_name=profile.name,
            signal_source=signal_source, tod_multiplier=tod_mult
        )

    @staticmethod
    def format_setup(ts: TradeSetup, fname: str, drift=None, vol_regime=None) -> str:
        dir_icon = "📈" if ts.direction == "BUY" else "📉"
        s_icon = {"STRONG": "🟢", "MODERATE": "🟡", "WEAK": "🔴"}.get(ts.signal_strength, "⬜")
        sg = '+' if ts.direction == "BUY" else '-'
        sgs = '-' if ts.direction == "BUY" else '+'
        return (
            f"*TRADE ALERT v4.3 — {fname}* {dir_icon} {_se(ts.direction)}\n\n"
            f"{s_icon} *{ts.direction}* {ts.signal_strength} {ts.signal_source}\n"
            f"Entry:  `{ts.entry:.6f}`\n"
            f"Target: `{ts.target:.6f}` ({sg}{ts.target_pct:.3f}%)\n"
            f"Stop:   `{ts.invalidation:.6f}` ({sgs}{ts.stop_pct:.3f}%)\n"
            f"{RISK_MSG}"
        )

# ============================================================================
# CONFLICT RESOLVER (unchanged)
# ============================================================================
class ConflictResolver:
    @staticmethod
    def resolve(sym: str, mc: MCResult, spike: Optional[SpikeAlert],
                trade: Optional[TradeSetup]) -> Tuple[Optional[TradeSetup], bool, str]:
        profile = effective_profile(sym)
        if spike is None:
...
class MCEngine:
    ...
    def _adaptive_targets(self, S0: float, sigma_tick: float,
    ...
    def _first_passage(paths, target_up, target_down, n_paths):
        ...
        for i in np.where(both)[0]:
            ...
            fw[i] = iu < id_
        p_up = (np.sum(hit_u & ~hit_d) + np.sum(fw)) / max(n_paths, 1)
        p_dn = (np.sum(hit_d & ~hit_u) + np.sum(both & ~fw)) / max(n_paths, 1)
        return p_up, p_dn

    def _edge_score(self, prob_hit_up: float, prob_hit_dn: float, fit_quality: float,
                    lam_deviation: float, is_jump: bool, ci_width_pct: float,
                    wr_weight: float, profile: AssetProfile,
                    drift: Optional[DriftSignal] = None,
                    vol_regime: Optional[VolRegimeResult] = None,
                    tod_mult: float = 1.0) -> Tuple[float, str]:

        regime_strong = ((vol_regime is not None and vol_regime.signal != "neutral" and vol_regime.edge_score >= 0.15) or
                         (drift is not None and drift.direction != "neutral" and drift.edge_score >= 0.15))
        ...
        mc_score = float(np.clip(skew * fit_quality * regime_bonus * wr_weight, 0.0, 1.0))
        mc_score = float(np.clip(mc_score + (tod_mult - 1.0) * 0.05 * mc_score, 0.0, 1.0))

        if vol_regime is not None and profile.vol_regime_enabled and vol_regime.signal != "neutral":
            w_v = profile.drift_edge_weight
            w_m = 1.0 - w_v
            score = float(np.clip(vol_regime.edge_score * w_v + mc_score * w_m, 0.0, 1.0))
            sig = vol_regime.signal if score >= profile.signal_edge_min else "neutral"
            return score, sig

        if drift is not None and profile.drift_signal_enabled and drift.direction != "neutral":
            w_d = profile.drift_edge_weight
            w_m = 1.0 - w_d
            score = float(np.clip(drift.edge_score * w_d + mc_score * w_m, 0.0, 1.0))
            sig = drift.direction if score >= profile.signal_edge_min else "neutral"
            return score, sig

        score = mc_score
        if gap >= profile.direction_gap and score >= profile.signal_edge_min:
    ...
    def run(self, sym: str, dm: DataManager, horizon: int, horizon_label: str,
            pm: PatternMemory, n: Optional[int] = None, timeframe_key: Optional[str] = None) -> Optional[MCResult]:
        profile = effective_profile(sym)
        ...
        try:
            drift = None
            vol_regime = None
            horizon_scaled_window = max(min(ns * 2, profile.drift_window), 30)
            ...
            if is_j:
                ...
                paths = _jd_kernel(
                    float(S0), float(jp.mu), float(jp.sigma), float(jp.lam_posterior),
                    float(jp.jump_mean), float(jp.jump_std), float(jp.jump_sign),
                    1.0, n_paths, ns, Z, U, Zj
                )
                pj = self.jm.prob_in_n(jp, ns)
                ej = self.jm.ticks_to_next_range(jp, dm.observed_tick_rate(sym))[0]
                jlo, jhi = self.jm.jump_magnitude_range(jp)
                sigma_tick = max(jp.sigma, 1e-10)
                fq = 0.80
                lam_dev = jp.lam_deviation
                if profile.drift_signal_enabled:
                    drift = self.de.analyse(sym, lr, S0, profile, tod_mult,
            ...
            if vol_regime is not None and profile.vol_regime_enabled and vol_regime.signal != "neutral":
                ...
                tup, tdn, sigma_h, tlabel = self._adaptive_targets(S0, sigma_tick, ns, profile)

            finals = paths[:, -1]
            prob_up_h = float(np.mean(finals > S0))
            prob_hit_up, prob_hit_dn = self._first_passage(paths, tup, tdn, n_paths)
            ...
            score, sig = self._edge_score(
                prob_hit_up, prob_hit_dn, fq, lam_dev, is_j,
                ci_width_pct, wr_weight, profile,
            ...
            if ENABLE_ENSEMBLE:
                try:
                    ...
                    ens_agreement = float(np.clip(hard_agree * 0.5 + soft_agree * 0.5, 0.0, 1.0))
            ...
            mc = MCResult(
                symbol=sym, S0=float(S0), horizon=ns, horizon_label=horizon_label,
                paths=paths,
                ...
                target_up=tup, target_down=tdn, target_label=tlabel,
                prob_up_horizon=prob_up_h, prob_hit_up=prob_hit_up, prob_hit_down=prob_hit_dn,
                sigma_horizon=sigma_h, ci_width_pct=float(ci_width_pct),
                prob_jump=pj, exp_jump_tick=ej,
                jump_magnitude_lo=jlo, jump_magnitude_hi=jhi,
                edge_score=score, signal=sig, implied_edge_pct=implied_edge,
                timeframe_key=timeframe_key,
                regime=(vol_regime.regime if vol_regime else (drift.regime if drift else regime)),
                profile_name=profile.name, drift_signal=drift, vol_regime=vol_regime,
                tod_multiplier=tod_mult, ens_agreement=ens_agreement
            )

            ts = self.tsb.build(mc, horizon_label, ns, mc.regime, profile,
                                drift, vol_regime, tod_mult)
            if ts is not None:
                try:
                    ...
                    ref_target, ref_stop, ref_method = self.htr.refine(
                        sym=sym, S0=float(S0), is_bull=is_bull_ts,
                        mc_target=ts.target, mc_stop=ts.invalidation,
                        finals=finals, prices=prices_for_refiner,
                        horizon=ns, records=pm.records
                    )
                    ref_target_pct = abs(ref_target - S0) / S0 * 100
                    ref_stop_pct = abs(ref_stop - S0) / S0 * 100
                    if ref_target_pct >= MIN_TARGET_MOVE * 100 and ref_stop_pct > 1e-6:
                        ref_rr = min(ref_target_pct / max(ref_stop_pct, 1e-10), 4.0)
...
# ============================================================================
# NARRATIVE ENGINE (unchanged – keep all methods, but not used for alerts)
# ============================================================================
class NarrativeEngine:
    ...
    def _vol_regime_block(self, vr: VolRegimeResult) -> str:
        dir_icon = "📈" if vr.signal == "bullish" else ("📉" if vr.signal == "bearish" else "⬜")
    ...
    def _drift_block(self, drift: Optional[DriftSignal]) -> str:
        if drift is None:
            return ""
        dir_icon = "📈" if drift.direction == "bullish" else ("📉" if drift.direction == "bearish" else "⬜")
    ...
    def vol_context(self, sym: str, gp: GBMParams, mc: MCResult, tf_label: str) -> str:
        profile = effective_profile(sym)
        ...
        lines.append(
            f"Expected move ({tf_label}): *+-{mc.sigma_horizon*100:.4f}%* (1-sigma)."
        )
        if mc.vol_regime:
        ...
        if mc.ens_agreement < 1.0:
            ...
            lines.append(
                f"\nEnsemble agreement: {ens_icon} `{mc.ens_agreement:.3f}`"
                + (" — signal degraded" if mc.ens_agreement < ENSEMBLE_AGREEMENT_FLOOR else "")
            )
        if mc.signal == "bullish":
            lines.append(f"\n*Signal — BULLISH* 🟢\nVol regime + momentum confirm UP. Edge: {mc.edge_score:.3f}")
        elif mc.signal == "bearish":
            lines.append(f"\n*Signal — BEARISH* 🔴\nVol regime + momentum confirm DOWN. Edge: {mc.edge_score:.3f}")
        else:
            lines.append("\n*Signal — NEUTRAL* ⬜\nVol regime not statistically significant. No trade setup.")
        if gp.ks_pvalue < 0.05:
            lines.append("\n*Fat tails detected* (KS p<0.05) — use wider stops.")
        if mc.trade_setup:
            ts = mc.trade_setup
            lines.append(
                f"\n*Trade setup:* {ts.direction} | R/R *1:{ts.rr_ratio:.2f}* | "
                f"P(tgt) *{ts.prob_target:.2%}* | EV `{ts.expected_value:+.4f}%` | [{ts.signal_source}]"
            )
        else:
            lines.append(
                f"\n*No valid trade setup.* Gates: P>={MIN_PROB_TARGET:.0%} EV>0 RR>={MIN_RR_GLOBAL:.1f} Move>={MIN_TARGET_MOVE*100:.2f}%."
            )
        return "\n".join(lines)

    def jump_context(self, sym: str, jp: JumpParams, mc: MCResult, tf_label: str) -> str:
        profile = effective_profile(sym)
        cat = _cat(sym)
        fname = _friendly(sym)
        ...
        lines = [
            ...
            f"Profile: *{profile.name}* | Regime: *{mc.regime}* | ToD: x{mc.tod_multiplier:.3f}\n",
        ]
        if cat == "jump":
            lines.append(
        ...
        if jp.counter_drift != 0.0:
            ...
            lines.append(f"Counter-drift: *{cd_dir}* slope={jp.counter_drift:.8f}")
        if mc.prob_jump is not None:
            lines.append(f"P(spike/{mc.horizon}t): *{mc.prob_jump:.1%}* (posterior λ)")
        if abs(jp.lam_deviation) > profile.spike_lam_dev_min:
            ...
            lines.append(
                ...
                + ("— spike may be overdue." if jp.lam_deviation < 0 else "— frequent spike regime.")
            )
        if mc.drift_signal:
            lines.append(self._drift_block(mc.drift_signal))
        if mc.ens_agreement < 1.0:
            ens_icon = "✅" if mc.ens_agreement >= ENSEMBLE_AGREEMENT_FLOOR else "⚠️"
            lines.append(f"\nEnsemble: {ens_icon} `{mc.ens_agreement:.3f}`" + (" — degraded" if mc.ens_agreement < ENSEMBLE_AGREEMENT_FLOOR else ""))
        if mc.signal == "bullish":
    ...
    def step_context(self, sym: str, gp: GBMParams, mc: MCResult, tf_label: str) -> str:
        lines = [
        ...
        if mc.ens_agreement < 1.0:
            lines.append(f"Ensemble: `{mc.ens_agreement:.3f}`" + (" ⚠️ degraded" if mc.ens_agreement < ENSEMBLE_AGREEMENT_FLOOR else ""))
        if mc.trade_setup:
            ts = mc.trade_setup
            lines.append(
                f"\n*Range Signal:* {ts.direction} via drift. "
                f"R/R *1:{ts.rr_ratio:.2f}* | EV `{ts.expected_value:+.4f}%`."
            )
        else:
            lines.append(
                "\n*No signal.* Step is near-random. "
                "Drift must be statistically significant with confirmed momentum."
            )
        return "\n".join(lines)
    ...
    def risk_reward_block(self, mc: MCResult, risk_pct: float) -> str:
        if mc.trade_setup:
            ...
            return (
                ...
                f"Entry:        `{ts.entry:.6f}`\n"
                f"Target:       `{ts.target:.6f}` ({'+' if ts.direction=='BUY' else '-'}{ts.target_pct:.3f}%)\n"
                f"Invalidation: `{ts.invalidation:.6f}` ({'-' if ts.direction=='BUY' else '+'}{ts.stop_pct:.3f}%)\n"
        ...
        return (
            ...
            f"(P>={MIN_PROB_TARGET:.0%} EV>0 RR>={MIN_RR_GLOBAL:.1f} Move>={MIN_TARGET_MOVE*100:.2f}%)."
        )

# ============================================================================
# CHART GENERATOR (unchanged – full version)
# ============================================================================
class ChartGen:
    def __init__(self):
        plt.rcParams.update({
    ...
    def make(self, fname: str, sym: str, dm: DataManager, mc: Optional[MCResult],
             gp: Optional[GBMParams], jp: Optional[JumpParams], vm: VolatilityModel,
             note: str, tod: TimeOfDayProfile) -> BytesIO:
        profile = effective_profile(sym)
        ...
        a1.plot(xh, recent, color=BL, lw=1.6, label="Live Price", zorder=5, alpha=0.95)
        a1.axvspan(xh[0], 0, alpha=0.05, color=BL)

        if mc is not None:
            ns = mc.horizon
            xf = np.arange(0, ns + 1)
            idx = np.random.choice(mc.paths.shape[0], min(40, mc.paths.shape[0]), replace=False)
            for i in idx:
                a1.plot(xf, mc.paths[i], color=PU, alpha=0.03, lw=0.5)
            a1.fill_between(xf, mc.p5, mc.p95, alpha=0.12, color=PU, label="90% CI", zorder=2)
            a1.fill_between(xf, mc.p25, mc.p75, alpha=0.25, color=BL, label="50% CI", zorder=3)
            a1.plot(xf, mc.p50, color=GR, lw=2.2, ls="--", label="Median", zorder=6)
            a1.axhline(mc.S0, color=YL, lw=0.8, ls=":", alpha=0.7, label="Entry", zorder=4)

            if mc.trade_setup:
                ts = mc.trade_setup
                tc = GR2 if ts.direction == "BUY" else RD2
                ic = RD2 if ts.direction == "BUY" else GR2
                a1.axhline(ts.target, color=tc, lw=2.0, ls="-", alpha=0.90, label="TARGET")
                a1.axhline(ts.invalidation, color=ic, lw=1.8, ls="--", alpha=0.85, label="STOP")
                xmid = ns * 0.55
                sg = '+' if ts.direction == 'BUY' else '-'
                a1.annotate(
                    f"TARGET {sg}{ts.target_pct:.2f}%\nRR 1:{ts.rr_ratio:.2f} EV:{ts.expected_value:+.3f}%\nP={ts.prob_target:.1%} Ens:{mc.ens_agreement:.2f}",
                    xy=(xmid, ts.target), fontsize=8, color=tc, fontweight="bold",
                    va="bottom" if ts.direction == "BUY" else "top",
                    bbox=dict(boxstyle="round,pad=0.2", fc=BG, ec=tc, alpha=0.85)
                )
                sg2 = '-' if ts.direction == 'BUY' else '+'
                a1.annotate(
                    f"STOP {sg2}{ts.stop_pct:.2f}%",
                    xy=(xmid, ts.invalidation), fontsize=8, color=ic, fontweight="bold",
                    va="top" if ts.direction == "BUY" else "bottom",
                    bbox=dict(boxstyle="round,pad=0.2", fc=BG, ec=ic, alpha=0.85)
                )
            else:
                a1.axhline(mc.target_up, color=GR2, lw=1.4, ls="-.", alpha=0.8, label="Upper")
                a1.axhline(mc.target_down, color=RD2, lw=1.4, ls="-.", alpha=0.8, label="Lower")

            a1.axvline(0, color=OR, lw=2.0, ls="--", alpha=0.9, label="NOW", zorder=7)

            if mc.vol_regime and mc.vol_regime.signal != "neutral":
                vr = mc.vol_regime
                arc = GR2 if vr.signal == "bullish" else RD2
                a1.annotate(
                    f"VOL_REGIME {'▲' if vr.signal=='bullish' else '▼'} k={vr.k_adaptive:.2f} dev={vr.vol_deviation:+.3f} Ens:{mc.ens_agreement:.2f}",
                    xy=(ns * 0.05, mc.S0), fontsize=8.5, color=arc, fontweight="bold",
                    bbox=dict(boxstyle="round,pad=0.3", fc=BG, ec=arc, alpha=0.90)
                )
            elif mc.drift_signal and mc.drift_signal.direction != "neutral":
                ds = mc.drift_signal
                arc = GR2 if ds.direction == "bullish" else RD2
                a1.annotate(
                    f"DRIFT {'▲' if ds.direction=='bullish' else '▼'} t={ds.tstat:+.2f} p={ds.pvalue:.3f} win={ds.window_used}",
                    xy=(ns * 0.05, mc.S0), fontsize=8.5, color=arc, fontweight="bold",
                    bbox=dict(boxstyle="round,pad=0.3", fc=BG, ec=arc, alpha=0.90)
                )
        ...
        regime_str = f" | {mc.regime}" if mc and mc.regime != "NORMAL" else ""
        a1.set_title(
            f"  SQE v4.3  ·  {fname}  ·  [{profile.name}]  ·  {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M UTC')}",
            fontsize=12, color=WH, loc="left", pad=12, fontweight="bold"
        )
        if mc:
            ...
            if mc.trade_setup:
                ts = mc.trade_setup
                ts_str = f"{ts.direction} RR1:{ts.rr_ratio:.2f} P={ts.prob_target:.1%} EV:{ts.expected_value:+.3f}% | "
            a1.set_title(
                f"{mc.horizon_label}{regime_str} | {ts_str}{mc.signal.upper()} | Edge {mc.edge_score:.3f} | src:{src} | Ens:{mc.ens_agreement:.2f} | ToD:x{mc.tod_multiplier:.3f}",
                fontsize=9, color=OR, loc="right", pad=12
            )
        a1.set_xlabel("Ticks (history | forecast)", fontsize=9, color=GY)
        a1.set_ylabel("Price", fontsize=9, color=GY)
        a1.legend(fontsize=7.5, framealpha=0.2, facecolor=BG, edgecolor=GY, ncol=7, loc="upper left")
        ...
        if len(lr) > 50:
            ...
            a2.hist(lc, bins=n_b, color=BL, alpha=0.65, density=True, label="Returns", edgecolor="none")
            if gp and gp.sigma > 1e-10:
                x = np.linspace(lc.min(), lc.max(), 500)
                a2.plot(x, norm.pdf(x, gp.mu, gp.sigma), color=GR, lw=2.0, label="GBM fit")
                thr = JUMP_THRESHOLD * gp.sigma
                a2.axvline(gp.mu + thr, color=RD, lw=0.9, ls="--", alpha=0.7, label="Jump thr")
                a2.axvline(gp.mu - thr, color=RD, lw=0.9, ls="--", alpha=0.7)
                if gp.mu != 0:
                    a2.axvline(gp.mu, color=YL, lw=1.5, ls="-", alpha=0.9, label=f"drift={gp.mu:.6f}")
            if is_j and jp:
                ij, _ = JumpDiffusionModel().detect(lc)
                jv = lc[ij]
                if len(jv) > 0:
                    a2.scatter(jv, np.zeros_like(jv), color=OR, s=20, zorder=5, label="Jumps")
        a2.set_title("Return Distribution", fontsize=9, color=GY, pad=5)
        ...
        if mc and mc.vol_regime:
            ...
            a3.set_title("Vol Regime Analysis", fontsize=9, color=GY, pad=5)
        else:
            self._drift_panel(a3, lr, mc, profile)
            a3.set_title("Drift t-stat Analysis", fontsize=9, color=GY, pad=5)
        ...
        a4.set_title("Probability Gauges", fontsize=9, color=GY, pad=5)
        ...
        a5.set_title("Time-of-Day Profile", fontsize=9, color=GY, pad=5)
        ...
        buf = BytesIO()
        plt.savefig(buf, format="png", dpi=115, bbox_inches="tight", facecolor=BG)
        plt.close(fig)
        buf.seek(0)
        return buf

    @staticmethod
    def _vol_regime_panel(ax, lr: np.ndarray, mc: Optional[MCResult], profile: AssetProfile):
        if len(lr) < 60:
            ax.text(0.5, 0.5, "Not enough data", ha="center", va="center", color=GY, transform=ax.transAxes)
            return
        ...
        for i in indices:
            ...
            zscores.append(float(np.clip(mu_s / max(se, 1e-12), -5, 5)))
        if not zscores:
            return
        ...
        if np.any(pos_sig):
            ax.bar(xvals[pos_sig], zscores[pos_sig], color=GR2, alpha=0.9, width=w)
        if np.any(neg_sig):
            ax.bar(xvals[neg_sig], zscores[neg_sig], color=RD2, alpha=0.9, width=w)
        if np.any(pos_ns):
            ax.bar(xvals[pos_ns], zscores[pos_ns], color=GR2, alpha=0.3, width=w)
        if np.any(neg_ns):
            ax.bar(xvals[neg_ns], zscores[neg_ns], color=RD2, alpha=0.3, width=w)
        ax.axhline(thr, color=YL, lw=1.2, ls="--", alpha=0.8, label=f"+{thr:.1f}")
        ax.axhline(-thr, color=YL, lw=1.2, ls="--", alpha=0.8, label=f"-{thr:.1f}")
        ax.axhline(0, color=GY, lw=0.8, alpha=0.5)
        if mc and mc.vol_regime:
            vr = mc.vol_regime
            col = GR2 if vr.signal == "bullish" else (RD2 if vr.signal == "bearish" else GY)
            ax.set_title(f"[{vr.signal.upper()}] z={vr.normalized_momentum:+.2f} k={vr.k_adaptive:.2f}", fontsize=8, color=col, pad=3)
        ax.legend(fontsize=7, framealpha=0.15, facecolor=BG)
        ax.set_ylabel("Mom z-score", fontsize=8)
        ax.set_xlabel("Tick index", fontsize=8)

    @staticmethod
    def _drift_panel(ax, lr: np.ndarray, mc: Optional[MCResult], profile: AssetProfile):
        if len(lr) < 60:
            ax.text(0.5, 0.5, "Not enough data", ha="center", va="center", color=GY, transform=ax.transAxes)
            return
        window = min(profile.drift_window, len(lr) // 3, 200)
        ...
        if np.any(pos_sig):
            ax.bar(xvals[pos_sig], tstats[pos_sig], color=GR2, alpha=0.9, width=w)
        if np.any(neg_sig):
            ax.bar(xvals[neg_sig], tstats[neg_sig], color=RD2, alpha=0.9, width=w)
        if np.any(pos_ns):
            ax.bar(xvals[pos_ns], tstats[pos_ns], color=GR2, alpha=0.3, width=w)
        if np.any(neg_ns):
            ax.bar(xvals[neg_ns], tstats[neg_ns], color=RD2, alpha=0.3, width=w)
        ...
        if mc and mc.drift_signal:
            ds = mc.drift_signal
            col = GR2 if ds.direction == "bullish" else (RD2 if ds.direction == "bearish" else GY)
            sig_str = " ✓ SIG" if ds.pvalue < 0.15 else " ✗ NS"
            ax.set_title(f"[{ds.direction.upper()}] t={ds.tstat:+.2f}{sig_str} win={ds.window_used}", fontsize=8, color=col, pad=3)
        ax.legend(fontsize=7, framealpha=0.15, facecolor=BG)
    ...
    def _tod_panel(ax, sym: str, tod: TimeOfDayProfile):
        ...
        ax.axhline(1.0, color=YL, lw=1.0, ls="--", alpha=0.7, label="Neutral")
        ax.axhline(TOD_MULT_MAX, color=GR2, lw=0.7, ls=":", alpha=0.5)
        ax.axhline(TOD_MULT_MIN, color=RD2, lw=0.7, ls=":", alpha=0.5)
        cur_h = _tod_bucket() / 2
        ax.axvline(cur_h, color=OR, lw=1.5, ls="-", alpha=0.9, label="NOW")
        ax.set_xlim(0, 24)
        ...
        ax.set_title(f"Now: x{b.multiplier:.3f} wr={b.win_rate:.0%} n={b.count:.0f}", fontsize=8, color=OR, pad=3)

    @staticmethod
    def _prob_box(ax, mc: MCResult, profile: AssetProfile):
        sig_c = {"bullish": GR, "bearish": RD, "jump_imminent": OR, "neutral": GY}.get(mc.signal, GY)
        ts_line = ""
        if mc.trade_setup:
            ts = mc.trade_setup
            ts_line = f"\n{ts.direction} RR1:{ts.rr_ratio:.2f} P={ts.prob_target:.1%} EV={ts.expected_value:+.3f}% [{ts.signal_strength}] src:{ts.signal_source}"
        else:
            ts_line = "\nNo valid setup (quality gates)"
        vr_line = ""
        if mc.vol_regime:
            vr = mc.vol_regime
            vr_line = f"\nVolReg: {vr.signal.upper()} z={vr.normalized_momentum:+.2f} k={vr.k_adaptive:.2f}"
        drift_line = ""
        if mc.drift_signal:
            ds = mc.drift_signal
            sig_icon = "✓" if ds.pvalue < 0.15 else "✗"
            drift_line = f"\nDrift: {ds.direction.upper()} t={ds.tstat:+.2f} p={ds.pvalue:.3f} {sig_icon} win={ds.window_used}"
        pj_line = ""
        if mc.prob_jump is not None:
            pj_line = f"\nP(spike {mc.horizon}t): {mc.prob_jump:.1%} (Bayesian λ)"
        ens_line = f"\nEnsemble: {mc.ens_agreement:.3f}" + (" ⚠️" if mc.ens_agreement < ENSEMBLE_AGREEMENT_FLOOR else " ✓")
        txt = (
            f"Signal: {mc.signal.upper()} [{profile.name}]\n"
            f"Edge: {mc.edge_score:.3f} | CI: {mc.ci_width_pct*100:.4f}%\n"
            f"P(up): {mc.prob_hit_up:.1%}  P(dn): {mc.prob_hit_down:.1%}"
            f"{vr_line}{drift_line}{pj_line}{ens_line}{ts_line}"
            f"\nToD: x{mc.tod_multiplier:.3f}"
        )
        ax.text(0.01, 0.97, txt, transform=ax.transAxes, fontsize=7.8, va="top", color=WH,
                fontfamily="monospace", bbox=dict(boxstyle="round,pad=0.45", fc="#0d1b2a", ec=sig_c, alpha=0.92))
    ...
    def _prob_gauges(ax, mc: MCResult, profile: AssetProfile):
        ...
        colors = [
            BL,
            (GR if mc.prob_hit_up > 0.55 else (RD if mc.prob_hit_up < 0.50 else YL)),
            (RD2 if mc.prob_hit_down > 0.55 else (GR if mc.prob_hit_down < 0.48 else YL)),
        ]
        if mc.vol_regime:
            vr = mc.vol_regime
            labels.append("VReg\nedge")
            vals.append(vr.edge_score)
            colors.append(GR if vr.signal == "bullish" else (RD if vr.signal == "bearish" else GY))
        elif mc.drift_signal:
            ds = mc.drift_signal
            labels.append("Drift\nedge")
            vals.append(ds.edge_score)
            colors.append(GR if ds.direction == "bullish" else (RD if ds.direction == "bearish" else GY))
        if mc.prob_jump is not None:
            labels.append(f"P(spike\n{mc.horizon}t)")
            vals.append(mc.prob_jump)
            colors.append(OR if mc.prob_jump > profile.spike_min_prob else GY)
        if mc.trade_setup:
            labels.append("R/R\nrating")
            rr_norm = min(mc.trade_setup.rr_ratio / 4.0, 1.0)
            vals.append(rr_norm)
            colors.append(GR if rr_norm > 0.5 else (YL if rr_norm > 0.3 else RD))
            labels.append("EV\n(norm)")
            ev_norm = float(np.clip(mc.trade_setup.expected_value / 0.5, 0.0, 1.0))
            vals.append(ev_norm)
            colors.append(GR if ev_norm > 0.3 else (YL if ev_norm > 0.1 else RD))
            labels.append("ToD\nmult")
            tod_norm = float(np.clip((mc.tod_multiplier - TOD_MULT_MIN) / (TOD_MULT_MAX - TOD_MULT_MIN), 0.0, 1.0))
            vals.append(tod_norm)
            colors.append(GR if mc.tod_multiplier > 1.05 else (RD if mc.tod_multiplier < 0.95 else GY))
        labels.append("Ensemble\nagree")
        vals.append(mc.ens_agreement)
        colors.append(GR if mc.ens_agreement >= 0.80 else (YL if mc.ens_agreement >= ENSEMBLE_AGREEMENT_FLOOR else RD))
        ...
        for bar, v, lbl in zip(bars, vals, labels):
            if "R/R" in lbl and mc.trade_setup:
                ...
                disp = f"{v:.1%}"
            ax.text(min(v + 0.03, 0.90), bar.get_y() + bar.get_height() / 2, disp, va="center", fontsize=8.5, color=WH, fontweight="bold")

    def _param_box(self, ax, sym, dm, mc, gp, jp, vm, note, profile: AssetProfile, tod: TimeOfDayProfile):
        S0v = dm.last(sym)
        s0s = f"{S0v:.6f}" if S0v else "N/A"
        rate = dm.observed_tick_rate(sym)
        b = tod._buckets.get(sym, [TodBucket()] * TOD_BUCKETS)[_tod_bucket()]
        info = [
            f"  | {REVERSE_MAP.get(sym,sym)} | PRICE={s0s} | TICKS={dm.n(sym):,} | RATE={rate:.2f}t/s | PROFILE={profile.name} | ENGINE="
            f"{'VOL_REGIME+MC' if profile.vol_regime_enabled else ('DRIFT+MC' if profile.drift_signal_enabled else 'MC+SPIKE')}"
            f" | {'LIVE' if dm._connected else 'RECON'} | SQEv4.3 | ToD_mult={b.multiplier:.3f} | ToD_wr={b.win_rate:.0%}"
        ...
        if gp and gp.n_obs > 0:
            ...
            dev = vm.deviation(gp)
            info.append(
                f"  | GBM mu={gp.mu:.8f} sig={gp.sigma:.8f} ewma={gp.sigma_ewma:.8f} ann={ann:.3f}% KS={gp.ks_pvalue:.4f} FQ={gp.fit_quality:.3f}"
                + (f" Adv={gp.advertised_vol*100:.0f}% Dev={dev:+.1f}%" if gp.advertised_vol else "")
            )
        if jp and jp.n_obs > 0:
            et = (1/jp.lam_posterior if jp.lam_posterior > 1e-10 else float("inf"))
            info.append(
                f"  | JUMP lam_mle={jp.lam:.6f} lam_post={jp.lam_posterior:.6f}(~1/{et:.0f}t) nj={jp.n_jumps} dev={jp.lam_deviation:+.1f}% hz={jp.hazard_intensity:.0%} sign={'UP' if jp.jump_sign > 0 else 'DN'}"
                f" p60={jp.spike_mag_p60*100:.3f}% p75={jp.spike_mag_p75*100:.3f}% cd={jp.counter_drift:.8f}"
            )
        if mc:
            ...
            if mc.trade_setup:
                ts = mc.trade_setup
                ts_s = f" | {ts.direction} RR1:{ts.rr_ratio:.2f} P={ts.prob_target:.2%} EV={ts.expected_value:+.4f}% [{ts.signal_strength}] src={ts.signal_source} tod=x{ts.tod_multiplier:.3f}"
            vr_s = ""
            if mc.vol_regime:
                vr = mc.vol_regime
                vr_s = f" | vr={vr.signal} z={vr.normalized_momentum:+.3f} k={vr.k_adaptive:.3f} dev={vr.vol_deviation:+.3f}"
            drift_s = ""
            if mc.drift_signal:
                ds = mc.drift_signal
                drift_s = f" | drift={ds.direction} t={ds.tstat:+.3f} p={ds.pvalue:.4f} mom={ds.ewma_momentum:+.3f} win={ds.window_used}"
            info.append(
                f"  | MC({mc.horizon}t) med={mc.p50[-1]:.6f} CI={mc.ci_width_pct*100:.4f}% 90w={rng:.4f}% edge={mc.edge_score:.4f} sig={mc.signal} ens={mc.ens_agreement:.3f}{ts_s}{vr_s}{drift_s}"
            )
        info += [
            f"  | {note}",
            f"  | Gates v4.3: P>={MIN_PROB_TARGET:.0%} EV>0 RR>={MIN_RR_GLOBAL:.1f} Move>={MIN_TARGET_MOVE*100:.2f}% Ens>={ENSEMBLE_AGREEMENT_FLOOR:.2f} STRONG/MODERATE | EmpCap p{EMPIRICAL_CAP_PERCENTILE}x{EMPIRICAL_CAP_MULTIPLIER} | ToD [{TOD_MULT_MIN},{TOD_MULT_MAX}]",
            "  | No model guarantees profit. Max 1-2% risk.",
        ]
        ax.text(0.004, 0.95, "\n".join(info), transform=ax.transAxes, fontsize=6.8, va="top", color=WH,
                fontfamily="monospace", bbox=dict(boxstyle="round,pad=0.45", fc=AX, ec=GY, alpha=0.93))

# ============================================================================
# ALERT ENGINE (with spike confidence filter)
# ============================================================================
class AlertEngine:
    ...
    def check_trade(self, sym: str, dm: DataManager, horizon: int, horizon_label: str) -> Optional[Tuple[float, str, MCResult]]:
        profile = effective_profile(sym)
        now = time.time()
        ...
        if r is None:
            return None
        if profile.vol_regime_enabled:
            if r.vol_regime is None or r.vol_regime.signal == "neutral":
                return None
        elif profile.drift_signal_enabled:
        ...
        self._save_last()
        log.info(
            f"Alert PASSED {sym}: {ts.direction} [{ts.signal_strength}] [{ts.signal_source}] "
            f"P={ts.prob_target:.2%} EV={ts.expected_value:+.4f}% RR=1:{ts.rr_ratio:.2f} Move={ts.target_pct:.3f}% Ens={r.ens_agreement:.3f} ToD=x{ts.tod_multiplier:.3f}"
        )
        return r.edge_score, r.signal, r

    def check_spike(self, sym: str, dm: DataManager, jm: JumpDiffusionModel,
                    se: SpikeEngine) -> Optional[SpikeAlert]:
        profile = effective_profile(sym)
        now = time.time()
        ...
        # Only send if confidence >= 95%
        if sa is not None and sa.confidence >= SPIKE_MIN_CONFIDENCE:
...
def _action_kb(sym: str, tf_key: str, cat: str) -> List[List[InlineKeyboardButton]]:
    rows = [
        _tf_kb(sym, "tf")[0],
        _tick_kb(sym, "tk")[0],
        [
            InlineKeyboardButton("🔄 Refresh", callback_data=f"tf:{sym}:{tf_key}"),
            InlineKeyboardButton("🎲 Prob Table", callback_data=f"prob_tf:{sym}:{tf_key}"),
            InlineKeyboardButton("⭐ Watch", callback_data=f"addwatch:{sym}"),
        ],
    ]
    if cat in ("boom", "crash"):
        rows.append([
            InlineKeyboardButton("⚡ Spike Check", callback_data=f"spike:{sym}"),
            InlineKeyboardButton("📈 Results", callback_data=f"results:{sym}"),
        ])
    else:
        rows.append([
            InlineKeyboardButton("📊 Drift/Regime", callback_data=f"drift:{sym}"),
            InlineKeyboardButton("📈 Results", callback_data=f"results:{sym}"),
...
class SQEBotV43:
    ...
    async def _on_tick(self, sym: str, price: float, ts: float):
        self.pm.resolve(sym, price, bot=self._bot_ref, chats=self._chats)
    ...
    async def cmd_start(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        # Simplified keyboard: only Spike Scanner and Alerts toggle
        kb = [
            [InlineKeyboardButton("🚨 Spike Scanner", callback_data="menu:spike_scan")],
            [InlineKeyboardButton("🔔 Alerts ON" if state.alerts_enabled else "🔕 Alerts OFF", callback_data="toggle:alerts")],
        ]
        st_icon = "🟢" if conn else "🔴"
        await update.message.reply_text(
            f"*SYNTHETIC QUANT ELITE v4.3*\n\n"
            ...
            "Use `/spike <symbol>` or quick commands like `/v75`, `/boom1000`",
            parse_mode=ParseMode.MARKDOWN,
            reply_markup=InlineKeyboardMarkup(kb)
        )

    # REMOVED: cmd_predict (no /predict command)

    async def cmd_spike(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        self._track(update)
        if not ctx.args:
            await update.message.reply_text("Usage: `/spike <symbol>`", parse_mode=ParseMode.MARKDOWN)
            return
    ...
    async def cmd_drift(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        self._track(update)
        if not ctx.args:
            await update.message.reply_text("Usage: `/drift <symbol>`", parse_mode=ParseMode.MARKDOWN)
            return
        sym = _resolve(" ".join(ctx.args))
        if not sym:
            await update.message.reply_text("Unknown symbol.")
    ...
    async def cmd_tod(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        self._track(update)
        if not ctx.args:
            await update.message.reply_text("Usage: `/tod <symbol>`", parse_mode=ParseMode.MARKDOWN)
            return
        sym = _resolve(" ".join(ctx.args))
        if not sym:
            await update.message.reply_text("Unknown symbol.")
    ...
    async def cmd_prob(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        self._track(update)
        if not ctx.args:
            await update.message.reply_text("Usage: `/prob <symbol>`", parse_mode=ParseMode.MARKDOWN)
            return
        sym = _resolve(" ".join(ctx.args))
        if not sym:
            await update.message.reply_text("Unknown symbol.")
    ...
    async def cmd_parameters(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        self._track(update)
        if not ctx.args:
            await update.message.reply_text("Usage: `/parameters <symbol>`", parse_mode=ParseMode.MARKDOWN)
            return
        sym = _resolve(" ".join(ctx.args))
        if not sym:
            await update.message.reply_text("Unknown symbol.")
    ...
    async def cmd_status(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        for fn, ds in ALL_SYMBOLS.items():
            ...
            engine = "VOL_REGIME" if profile.vol_regime_enabled else ("DRIFT+MC" if profile.drift_signal_enabled else "MC+SPIKE")
            tod_m = self.tod.multiplier(ds)
            ok = "✅" if n >= MIN_TICKS else f"({n})"
            lines.append(
                f"{fn:<14} {n:>7,} {rate:>6.2f}t/s {profile.name:>8} {engine:>12} x{tod_m:.2f} {ok}"
            )
    ...
    async def cmd_results(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        self._track(update)
        if not ctx.args:
            await update.message.reply_text("Usage: `/results <symbol>`", parse_mode=ParseMode.MARKDOWN)
            return
        sym = _resolve(" ".join(ctx.args))
        if not sym:
            await update.message.reply_text("Unknown symbol.")
    ...
    async def cmd_unwatch(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        self._save_states()
        await update.message.reply_text(
            f"Removed. Watchlist: {', '.join(_friendly(s) for s in st.watchlist) or 'empty'}",
            parse_mode=ParseMode.MARKDOWN
        )
    ...
    async def cmd_quick_sym(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        if cat in ("boom", "crash"):
            kb.append([InlineKeyboardButton("⚡ Spike Check", callback_data=f"spike:{sym}")])
        engine_str = "VOL_REGIME+MC" if profile.vol_regime_enabled else ("DRIFT+MC" if profile.drift_signal_enabled else "MC+SPIKE")
        tod_m = self.tod.multiplier(sym)
        await update.message.reply_text(
            f"*{_friendly(sym)}* [{profile.name}] Engine: *{engine_str}* | ToD: x{tod_m:.3f}\n"
            f"Select timeframe:",
            parse_mode=ParseMode.MARKDOWN,
            reply_markup=InlineKeyboardMarkup(kb)
        )
        ...
        try:
            if d.startswith("menu:"):
                ...
                if sym not in st.watchlist:
                    st.watchlist.append(sym)
                self._save_states()
                await q.message.reply_text(f"⭐ *{_friendly(sym)}* added!", parse_mode=ParseMode.MARKDOWN)
                ...
                if sym in st.watchlist:
                    st.watchlist.remove(sym)
                self._save_states()
                await q.message.reply_text(f"Removed *{_friendly(sym)}*.", parse_mode=ParseMode.MARKDOWN)
            elif d.startswith("risk:"):
                pct = float(d[5:])
                self._state(cid).risk_pct = pct
                self._save_states()
                await q.message.reply_text(f"Risk: *{pct:.1f}%*", parse_mode=ParseMode.MARKDOWN)
                ...
                await q.message.reply_text(
                    f"Alerts *{s}*.\n"
                    + (f"Gates v4.3:\n  P>={MIN_PROB_TARGET:.0%} EV>0\n  RR>={MIN_RR_GLOBAL:.1f} (dynamic)\n  Ensemble>={ENSEMBLE_AGREEMENT_FLOOR:.2f}\n  EmpCap p{EMPIRICAL_CAP_PERCENTILE}x{EMPIRICAL_CAP_MULTIPLIER}\n  STRONG/MODERATE only"
    ...
    async def _do_analysis(self, cid: int, ctx, sym: str, horizon: int, hlabel: str, tf_key: Optional[str] = None):
        fname = _friendly(sym)
        n = self.dm.n(sym)
        S0 = self.dm.last(sym)
        profile = effective_profile(sym)
        ...
        if n < MIN_TICKS or S0 is None:
            await ctx.bot.send_message(
                cid,
                f"*{fname}* [{profile.name}]: {n} ticks (need {MIN_TICKS}+). Retry in ~30s.",
                parse_mode=ParseMode.MARKDOWN
            )
            return
        ...
        if cat in ("vol", "step"):
            ...
            jp = self.jm.fit_unbiased(sym, lr)
            gp = GBMParams(mu=jp.mu, sigma=jp.sigma, sigma_ewma=jp.sigma, n_obs=jp.n_obs, fit_quality=0.80)

        loop = asyncio.get_event_loop()
        mc = await loop.run_in_executor(None, self.mce.run, sym, self.dm, horizon, hlabel, self.pm, None, tf_key)
        if mc is None:
            await ctx.bot.send_message(cid, f"Analysis failed for {fname}. Retry.", parse_mode=ParseMode.MARKDOWN)
            return
        ...
        validated_trade, trade_valid, reason = self.cr.resolve(sym, mc, spike_alert, mc.trade_setup)
        mc.trade_setup = validated_trade if trade_valid else None

        if mc.signal not in ("neutral",):
            tup = mc.trade_setup.target if mc.trade_setup else mc.target_up
            tdn = mc.trade_setup.invalidation if mc.trade_setup else mc.target_down
            self.pm.record(sym, mc.horizon, mc.signal, max(mc.prob_hit_up, mc.prob_hit_down),
                           mc.edge_score, S0, tup, tdn)

        note = self.pm.note(sym, mc.horizon)
        st = self._state(cid)

        img = await loop.run_in_executor(None, self.cg.make, fname, sym, self.dm, mc, gp, jp, self.vm, note, self.tod)
        cap = self._caption(fname, sym, mc, gp, jp, note, st.risk_pct)
        used_tf = tf_key or "5m"
        kb = _action_kb(sym, used_tf, cat)

        await _safe_send_photo(ctx.bot, cid, photo=img, caption=cap, reply_markup=InlineKeyboardMarkup(kb))

        full_text = self._full_analysis(fname, sym, mc, gp, jp, note, st.risk_pct, hlabel)
        if full_text:
            await _safe_send_message(ctx.bot, cid, full_text)

        if spike_alert and spike_alert.confidence >= SPIKE_MIN_CONFIDENCE:
            await _safe_send_message(ctx.bot, cid, self.se.format_standalone(spike_alert))

        if not trade_valid and reason == "spike_primary_conflict":
            await _safe_send_message(ctx.bot, cid, f"*Note:* Trade suppressed — Bayesian spike direction takes priority.")
    ...
    async def _do_tod_report(self, cid: int, ctx, sym: str):
        ...
        lines = [
            ...
            f"Current: UTC {cur_b//2:02d}:{'30' if cur_b%2 else '00'} (bucket {cur_b})\n",
            f"*Current bucket:*",
            self.tod.summary(sym), "",
            f"*Best 5 (by multiplier):*",
        ]
        sorted_b = sorted(enumerate(buckets), key=lambda x: x[1].multiplier, reverse=True)[:5]
        for idx, b in sorted_b:
            h = idx // 2
            mins = "30" if idx % 2 else "00"
            icon = "🟢" if b.multiplier >= 1.05 else "🟡"
            lines.append(f"{icon} {h:02d}:{mins} UTC | x{b.multiplier:.3f} | wr={b.win_rate:.0%} | n={b.count:.0f}")
        lines += ["", "*Worst 3:*"]
        worst_b = sorted(enumerate(buckets), key=lambda x: x[1].multiplier)[:3]
        for idx, b in worst_b:
            h = idx // 2
            mins = "30" if idx % 2 else "00"
            lines.append(f"🔴 {h:02d}:{mins} UTC | x{b.multiplier:.3f} | wr={b.win_rate:.0%} | n={b.count:.0f}")
        lines += [
            "", "*How ToD works (v4.3):*",
    ...
    async def _do_drift_report(self, cid: int, ctx, sym: str):
        ...
        profile = effective_profile(sym)
        if n < MIN_TICKS:
            await ctx.bot.send_message(cid, f"{fname}: {n} ticks — need {MIN_TICKS}+.", parse_mode=ParseMode.MARKDOWN)
            return
        lr = self.dm.log_returns(sym)
        S0 = self.dm.last(sym) or 1.0
        tod_mult = self.tod.multiplier(sym)
        default_horizon = 300
        horizon_scaled_window = max(min(default_horizon * 2, profile.drift_window), 30)

        if profile.vol_regime_enabled:
            prices_arr = self.dm.prices(sym)
            vr = self.vre.analyse(sym, lr, S0, default_horizon, profile, tod_mult,
                                   window_override=horizon_scaled_window, prices=prices_arr)
            dir_icon = "📈" if vr.signal == "bullish" else ("📉" if vr.signal == "bearish" else "⬜")
            sig_icon = "✅ CONFIRMED" if vr.signal != "neutral" else "❌ NOT CONFIRMED"
            lines = [
                f"*Vol Regime Report — {fname}* [{profile.name}]\n",
                f"Engine: *VOL_REGIME+MC* | ToD: x{tod_mult:.3f} | Window: {horizon_scaled_window}t\n",
                ...
                f"  Deviation:  `{vr.vol_deviation:+.4f}` ({'above' if vr.vol_deviation > 0 else 'below'} target)",
                f"  k_adaptive: `{vr.k_adaptive:.4f}`\n",
                f"*Momentum (blended z-score):*",
                f"  z-score: `{vr.normalized_momentum:+.4f}`" + (" ✅" if abs(vr.normalized_momentum) >= profile.drift_tstat_min * 0.60 else " ❌"),
                f"  V-Edge:  `{vr.edge_score:.4f}`",
                f"  ToD adj: ±{abs(self.tod.prob_adjustment(sym))*100:.2f}%\n",
                f"*Trade Levels (k={vr.k_adaptive:.2f}):*",
                f"  Target UP:   `{vr.target_up:.6f}` (+{(vr.target_up-S0)/S0*100:.3f}%)",
                f"  Target DOWN: `{vr.target_down:.6f}` (-{(S0-vr.target_down)/S0*100:.3f}%)",
                f"  Stop UP:     `{vr.stop_up:.6f}`",
                f"  Stop DOWN:   `{vr.stop_down:.6f}`",
            ]
            if vr.signal != "neutral":
                rr = min(abs(vr.target_up - S0) / max(abs(S0 - vr.stop_down), 1e-10), 4.0)
                lines += [
                ...
                lines += ["\n*Signal NOT YET CONFIRMED*", "Momentum not significant or regime unfavorable.", "Monitor and check again later."]
        else:
            ds = self.de.analyse(sym, lr, S0, profile, tod_mult, window_override=horizon_scaled_window)
            dir_icon = "📈" if ds.direction == "bullish" else ("📉" if ds.direction == "bearish" else "⬜")
            sig_icon = "✅ CONFIRMED" if ds.direction != "neutral" else "❌ NOT CONFIRMED"
            filled = int(abs(ds.ewma_momentum) * 10)
            mom_bar = "█" * filled + "░" * (10 - filled)
            lines = [
                f"*Drift Report — {fname}* [{profile.name}]\n",
                f"Engine: *DRIFT+MC* | ToD: x{tod_mult:.3f} | Window: {horizon_scaled_window}t\n",
                f"Direction: *{ds.direction.upper()}* {dir_icon} {sig_icon}",
                f"Strength:  *{ds.signal_strength}*\n",
                f"*Statistics:*",
                f"  t-stat:  `{ds.tstat:+.6f}` (thresh={profile.drift_tstat_min:.1f})" + (" ✓" if abs(ds.tstat) >= profile.drift_tstat_min else " ✗"),
                f"  p-value: `{ds.pvalue:.6f}`" + (" ✓" if ds.pvalue < 0.15 else " ✗ (need < 0.15)"),
                f"  EWMA mom:[{mom_bar}] `{ds.ewma_momentum:+.4f}`" + (" ✓" if abs(ds.ewma_momentum) > 0.05 else " ✗"),
                f"  Regime:  *{ds.regime}*",
            ...
            if ds.direction != "neutral":
                lines += [f"\n*Signal CONFIRMED* {dir_icon}", f"Direction: {ds.direction.upper()}", f"Run `/predict {fname}` for full trade setup."]
            else:
                lines += ["\n*Signal NOT YET CONFIRMED*", "Not all conditions met.", "Monitor and check again later."]
    ...
    async def _do_spike_check(self, cid: int, ctx, sym: str):
        ...
        if sa and sa.confidence >= SPIKE_MIN_CONFIDENCE:
            report = self.se.format_standalone(sa)
            S0 = self.dm.last(sym) or 0.0
            self.pm.record(sym, 300, "spike_imminent", sa.confidence, sa.confidence,
                           S0, S0 * (1 + sa.magnitude_p75 / 100), S0 * (1 - sa.magnitude_p75 / 100))
        else:
            pp = self.jm.prob_in_n(jp, 300)
            report = f"*Spike Check — {fname}*\n\nStatus: ⬜ NOT IMMINENT\nP(spike 300t): {pp:.1%}\nConfidence threshold: ≥{SPIKE_MIN_CONFIDENCE:.0%}\n{RISK_MSG}"
        kb = [[
            InlineKeyboardButton("📊 Full Analysis", callback_data=f"sym:{sym}"),
            InlineKeyboardButton("🔄 Refresh", callback_data=f"spike:{sym}"),
        ]]
        await _safe_send_message(ctx.bot, cid, report, reply_markup=InlineKeyboardMarkup(kb))

    async def _do_spike_scan(self, cid: int, ctx):
        await ctx.bot.send_message(cid, "🔍 Scanning Boom/Crash (Bayesian v4.3) ...", parse_mode=ParseMode.MARKDOWN)
        results = []
        ...
        for fn, ds in scan_syms.items():
            ...
            try:
                ...
                if sa and sa.confidence >= SPIKE_MIN_CONFIDENCE:
                    results.append((fn, ds, sa))
            except Exception:
                continue
        if not results:
            await ctx.bot.send_message(cid, "No imminent spike with sufficient confidence.")
            return
        lines = ["*🚨 Spike Scanner — Imminent (≥95% confidence):*\n"]
        for fn, ds, sa in results[:5]:
            dir_icon = "📈" if sa.direction == "up" else "📉"
            lines.append(f"{dir_icon} `{fn}` | Conf:{sa.confidence:.0%} | Window:{sa.time_lo_str}–{sa.time_hi_str}")
        kb = [[InlineKeyboardButton(fn, callback_data=f"spike:{ds}")] for fn, ds, _ in results[:5]]
        await _safe_send_message(ctx.bot, cid, "\n".join(lines), reply_markup=InlineKeyboardMarkup(kb))

    async def _send_prob_table(self, cid: int, ctx, sym: str, horizon: int, hlabel: str):
        fname = _friendly(sym)
        S0 = self.dm.last(sym)
        n = self.dm.n(sym)
        profile = effective_profile(sym)
        ...
        for tf_key, tf_label in TIMEFRAMES.items():
            try:
                ...
                r = self.mce.run(sym, self.dm, ticks, lbl, self.pm, n=3000, timeframe_key=tf_key)
                if r:
                    results[tf_key] = r
            except Exception:
                pass
        ...
        lines = [
            ...
            f"{'TF':<8}|{'P(up)':>7}|{'P(+)':>7}|{'P(-)':>7}|{'Edge':>6}|{'RR':>7}|{'EV':>7}|{'Str':>4}|{'Ens':>6}",
            "─" * 72,
        ]
        for tf_key, r in results.items():
            rr_str = f"1:{r.trade_setup.rr_ratio:.1f}" if r.trade_setup else "—"
            st_str = r.trade_setup.signal_strength[:3] if r.trade_setup else "—"
            ev_str = f"{r.trade_setup.expected_value:+.3f}" if r.trade_setup else "—"
            ens_str = f"{r.ens_agreement:.2f}"
            se_i = _se(r.signal)
            lines.append(
                f"{TIMEFRAMES[tf_key]:<8}|{r.prob_up_horizon:>7.1%}|{r.prob_hit_up:>7.1%}|{r.prob_hit_down:>7.1%}|"
                f"{r.edge_score:>6.3f}|{rr_str:>7}|{ev_str:>7}|{se_i}{st_str}|{ens_str:>6}"
            )
        if results:
            best_tf = max(results, key=lambda k: results[k].edge_score)
            br = results[best_tf]
            lines += ["", f"*Best:* {TIMEFRAMES[best_tf]} | {_se(br.signal)} {br.signal.upper()} | Edge {br.edge_score:.3f} | Ens {br.ens_agreement:.3f}"]
            if br.trade_setup:
                ts = br.trade_setup
                lines.append(f"Setup: *{ts.direction}* | RR *1:{ts.rr_ratio:.2f}* | P *{ts.prob_target:.2%}* | EV `{ts.expected_value:+.4f}%` | *{ts.signal_strength}* [{ts.signal_source}] ToD:x{ts.tod_multiplier:.3f}")
            if br.vol_regime:
                vr = br.vol_regime
                lines.append(f"VolReg: {vr.signal.upper()} | z={vr.normalized_momentum:+.3f} | k={vr.k_adaptive:.3f} | dev={vr.vol_deviation:+.3f}")
            elif br.drift_signal:
                ds = br.drift_signal
                lines.append(f"Drift: {ds.direction.upper()} | t={ds.tstat:+.3f} | p={ds.pvalue:.4f} | mom={ds.ewma_momentum:+.3f} | win={ds.window_used}")
        lr = self.dm.log_returns(sym)
        cat = _cat(sym)
        narr = ""
        try:
            ref = results.get("5m") or (list(results.values())[0] if results else None)
            if ref:
                if cat in ("vol", "step"):
                    gp = self.vm.fit(sym, lr)
                    narr = self.narr.step_context(sym, gp, ref, ref.horizon_label) if cat == "step" else self.narr.vol_context(sym, gp, ref, ref.horizon_label)
                else:
                    jp = self.jm.fit_unbiased(sym, lr)
    ...
    async def _send_parameters(self, cid: int, ctx, sym: str):
        ...
        S0 = self.dm.last(sym) or 1.0
        tod_m = self.tod.multiplier(sym)
        horizon_scaled_window = max(min(300 * 2, profile.drift_window), 30)
        lines = [
            f"*Parameters: {fname}* [{profile.name}]\n",
            f"Category: `{cat.upper()}` | Ticks: `{n:,}` | Rate: `{rate:.2f}t/s` | ToD: x{tod_m:.3f}\n",
            f"*Profile v4.3:*\n"
        ...
        if len(lr) < MIN_TICKS:
            ...
            if cat in ("vol", "step"):
                ...
                lines += [
                    ...
                    f"Ann vol:   `{a:.5f}%`",
                    f"KS pval:   `{p.ks_pvalue:.5f}`" + (" normal" if p.ks_pvalue > 0.05 else " fat-tailed"),
                    f"Fit qual:  `{p.fit_quality:.4f}`",
                ]
                if p.advertised_vol:
                    lines.append(f"Adv vol:   `{p.advertised_vol*100:.0f}%` | Dev: `{d:+.2f}%`")
                prices_arr = self.dm.prices(sym)
                emp_cap = _empirical_move_cap(prices_arr, 300)
                if emp_cap is not None:
                    lines.append(f"EmpCap(300t): `{emp_cap*100:.3f}%` (p{EMPIRICAL_CAP_PERCENTILE}x{EMPIRICAL_CAP_MULTIPLIER})")
                if profile.vol_regime_enabled:
                    prices_arr = self.dm.prices(sym)
                    vr = self.vre.analyse(sym, lr, S0, 300, profile, tod_m, window_override=horizon_scaled_window, prices=prices_arr)
                    lines += [
                        f"\n*Vol Regime Engine v4.3:*",
            ...
            if profile.drift_signal_enabled:
                ds = self.de.analyse(sym, lr, S0, profile, tod_m, window_override=horizon_scaled_window)
                lines += [
                    f"\n*Drift Analysis:*",
    ...
    async def _send_results(self, cid: int, ctx, sym: str):
        ...
        tod_m = self.tod.multiplier(sym)
        if n == 0:
            await ctx.bot.send_message(cid, f"*Results — {fname}* [{profile.name}]\n\nNo resolved predictions yet.\nv4.3 quality gates: fewer but higher-quality signals.\nToD: x{tod_m:.3f}", parse_mode=ParseMode.MARKDOWN)
            return
        wins = sum(1 for r in self.pm.records if r.symbol == sym and r.resolved and r.correct)
        losses = n - wins
        icon = "🟢" if wr >= 0.55 else ("🔴" if wr < 0.45 else "🟡")
        msg = f"*Results — {fname}* [{profile.name}]\n\n*Accuracy:* {icon} *{wr:.0%}*\nW:{wins} L:{losses} Total:{n}\nAvg edge: {ae:.3f} | ToD now: x{tod_m:.3f}\n\n{recent}"
        await _safe_send_message(ctx.bot, cid, msg)

    def _caption(self, fname: str, sym: str, mc: MCResult, gp: Optional[GBMParams],
                 jp: Optional[JumpParams], note: str, risk_pct: float) -> str:
        S0 = mc.S0
        n = self.dm.n(sym)
        se_i = _se(mc.signal)
        profile = effective_profile(sym)
        L = [
            ...
            f"Price: `{_safe_md(S0,'.6f')}` | Ticks: `{n:,}` | ToD: x{mc.tod_multiplier:.3f} | Ens: {mc.ens_agreement:.2f}",
        ]
        if mc.trade_setup:
            ts = mc.trade_setup
            di = "📈" if ts.direction == "BUY" else "📉"
            si = {"STRONG": "🟢", "MODERATE": "🟡", "WEAK": "🔴"}.get(ts.signal_strength, "⬜")
            sg = '+' if ts.direction == 'BUY' else '-'
            sgs = '-' if ts.direction == 'BUY' else '+'
            L += [
                f"\n{si} *{ts.direction}* {di} [{ts.signal_strength}] [{ts.signal_source}]\n",
            ...
            L += [
                ...
                f"Edge:`{_safe_md(mc.edge_score,'.4f')}` CI:`{_safe_md(mc.ci_width_pct*100,'.4f')}%`",
                "No trade setup — quality gates not met.",
            ]
        if mc.vol_regime and mc.vol_regime.signal != "neutral":
            vr = mc.vol_regime
            L.append(f"VolReg: `{vr.signal}` z=`{vr.normalized_momentum:+.3f}` k=`{vr.k_adaptive:.3f}`")
        elif mc.drift_signal:
            ds = mc.drift_signal
            sig_i = "✅" if ds.direction != "neutral" else "❌"
            L.append(f"Drift: `{ds.direction}` t=`{ds.tstat:+.3f}` p=`{ds.pvalue:.3f}` {sig_i} win={ds.window_used}")
        if mc.prob_jump is not None:
            L.append(f"P(spike/{mc.horizon}t): `{mc.prob_jump:.2%}` (Bayesian λ)")
        L += ["", note, RISK_MSG]
        return "\n".join(line for line in L if line is not None)

    def _full_analysis(self, fname: str, sym: str, mc: MCResult, gp: Optional[GBMParams],
                       jp: Optional[JumpParams], note: str, risk_pct: float, hlabel: str) -> str:
        cat = _cat(sym)
        parts = []
        if mc.trade_setup:
            parts.append(TradeSetupBuilder.format_setup(mc.trade_setup, fname, mc.drift_signal, mc.vol_regime))
        try:
    ...
    async def _alert_loop(self, bot):
        ...
        log.info(
            f"Alert scanner v4.3 active. Gates: P>={MIN_PROB_TARGET:.0%} EV>0 RR>={MIN_RR_GLOBAL:.1f} Move>={MIN_TARGET_MOVE*100:.2f}% Ens>={ENSEMBLE_AGREEMENT_FLOOR:.2f} EmpCap p{EMPIRICAL_CAP_PERCENTILE}x{EMPIRICAL_CAP_MULTIPLIER}"
        )
        scan_count = 0
        ...
        while True:
            try:
                ...
                if not self.dm._init_done:
                    continue
                enabled = [c for c in self._chats if self._state(c).alerts_enabled]
                if not enabled:
                    continue
                ...
                if profile.spike_enabled:
                    ...
                    if len(lr) >= MIN_TICKS:
                        jp = self.jm.fit_unbiased(ds, lr)
                        tr = self.dm.observed_tick_rate(ds)
                        spike_sa = self.se.assess(ds, jp, 300, tr, self.tod.multiplier(ds))
                validated, tv, _ = self.cr.resolve(ds, mc, spike_sa, mc.trade_setup)
                if not tv or validated is None:
                    continue
                ts = validated
                msg = TradeSetupBuilder.format_setup(ts, fn, mc.drift_signal, mc.vol_regime)
                for cid in enabled:
                    try:
                        await _safe_send_message(bot, cid, msg)
                        await asyncio.sleep(0.1)
    ...
    def run(self):
        app = Application.builder().token(TELEGRAM_TOKEN).post_init(self._post_init).post_stop(self._post_stop).build()
        for cmd in ["v10","v25","v50","v75","v100","v250","boom300","boom500","boom600","boom900","boom1000",
                    "crash300","crash500","crash600","crash900","crash1000","jump10","jump25","jump50","jump75","jump100","step","stepindex"]:
        ...
        app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, self._track_msg))
...
if __name__ == "__main__":
    log.info("=" * 66)
    log.info("  SYNTHETIC QUANT ELITE v4.3 — PRODUCTION (MINIMAL MESSAGES)")
    log.info("=" * 66)
    log.info(f"  Global gates v4.3:")
    log.info(f"    P>={MIN_PROB_TARGET:.0%} (cap 72%)  EV>0 per-asset  RR>={MIN_RR_GLOBAL:.1f} (dynamic cap 4.0)")
    log.info(f"    Stop>=30% target  Move>={MIN_TARGET_MOVE*100:.2f}%  Ens>={ENSEMBLE_AGREEMENT_FLOOR:.2f}")
    log.info(f"    EmpCap p{EMPIRICAL_CAP_PERCENTILE}x{EMPIRICAL_CAP_MULTIPLIER}  HybridRefiner cold<{REFINER_MIN_PATTERNS}")
    log.info(f"  ToD: {TOD_BUCKETS} buckets  alpha={TOD_ALPHA}  range=[{TOD_MULT_MIN},{TOD_MULT_MAX}]  adj=±3% max")
    log.info("=" * 66)
    for name, profile in [("VOL", PROFILE_VOL), ("BOOM", PROFILE_BOOM), ("CRASH", PROFILE_CRASH), ("JUMP", PROFILE_JUMP), ("STEP", PROFILE_STEP)]:
        engine = "VOL_REGIME+MC" if profile.vol_regime_enabled else ("DRIFT+MC" if profile.drift_signal_enabled else "MC+SPIKE")
        log.info(f"  [{name:<5}] {engine:<14} edge>={profile.alert_edge_min:.2f} RR>={profile.alert_min_rr:.1f} cool={profile.alert_cooldown:.0f}s spike={profile.spike_enabled} bias={profile.direction_bias} str={profile.alert_strength}")
    log.info("=" * 66)
